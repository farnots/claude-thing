...
function contextMeter(fraction) {
  ...
  return '<div class="dctx' + (pct >= 0.8 ? ' hot' : '') + '">' +
    ...
    '<span class="dctxlabel">' + t('detail.contextUsed') + '</span></div>' +
...
export function renderDetail(state, id) {
  ...
  return '<div class="screen">' + topbar(t('bar.session'), state.daemonConnected) +
    ...
    '<div class="activity">' + esc(d.currentTool
      ? t('detail.toolWorking', { tool: d.currentTool })
      : t('detail.noTool')) + '</div>' +
    '<div class="lasttext">' + esc(d.lastMessage || '') + '</div>' +
    contextMeter(d.context) +
    '<div class="statsrow">' +
    '<div class="stat"><div class="v">' + fmtTokens(d.tokens.out) + '</div><div class="k">' + t('detail.tokensOut') + '</div></div>' +
    '<div class="stat"><div class="v">' + fmtTokens(d.tokens.in) + '</div><div class="k">' + t('detail.tokensIn') + '</div></div>' +
    '<div class="stat"><div class="v">' + fmtTokens(d.cacheRead) + '</div><div class="k">' + t('detail.cacheRead') + '</div></div>' +
    '<div class="stat"><div class="v">' + t('state.' + stateLabel(d.state, d.ended)) + '</div><div class="k">' + t('detail.state') + '</div></div>' +
    '</div></div></div>';
}
